...
const Blogs = () => {
  ...
  const fetchData = async () =>{
    try {
      ...
      const nine = Math.ceil(data.data.length / 2 +9);
      setBlogs(data.data.slice(half, nine))
...
useEffect(() =>{
  ...
  return (
    <section className="py-20 px-6 bg-black">
      <h3 className="text-3xl font-bold text-[#fff] mb-12 text-center">Latest Blogs</h3>
      <div className="grid md:grid-cols-3 gap-8">
        {blogs.map((blog, index) => (<div key={index} className="bg-gray-800 p-6 rounded-xl shadow-md">
